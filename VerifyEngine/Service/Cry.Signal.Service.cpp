...
namespace Cry
{
	namespace Signal
	{
		...
		bool Work::CheckOnline(w32 wIndex, const std::string & Name, const std::string & Pass)
		{
			if (wIndex < 0)
			{
				return false;
			}
			return this->CheckOnline({ wIndex, Name, Pass });
		}
		...
		bool NetworkServiceEngine::CreateService()
		{
			if (m_Services != nullptr)
			{
				if (!m_Services->Init())
				{
					CryMessage("·þÎñÆ÷³õÊ¼Ê§°Ü");
					return false;
				}
				if (!m_Services->Start())
				{
					CryMessage("·þÎñÆ÷Æô¶¯Ê§°Ü");
					return false;
				}
				if (!m_Loop->Start())
				{
					CryMessage("·þÎñÆ÷¼àÌýÊ§°Ü");
					return false;
				}
				return true;
			}
